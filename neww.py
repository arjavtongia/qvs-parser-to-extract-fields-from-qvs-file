...
“”“Remove /* … */ block comments and // line comments from QVS text.”””
# Remove block comments /* … */ (non-greedy, dotall)
text = re.sub(r’/*.*?*/’, ‘’, text, flags=re.DOTALL)
# Remove // line comments — but only outside of field definitions
# We remove from // to end of line
text = re.sub(r’//[^\n]*’, ‘’, text)
return text
...
```
# Protect ///$ tab markers before stripping comments (they look like // comments)
qvs_text = re.sub(
    r'///\$tab\s+(.+)',
    lambda m: f'<<<TABMARKER:{m.group(1).strip()}>>>',
    qvs_text
)
# Strip /* ... */ block comments and // line comments
qvs_text = strip_comments(qvs_text)
# Restore tab markers
qvs_text = re.sub(
    r'<<<TABMARKER:(.+?)>>>',
...
# Comments already stripped upstream in parse_qvs
stmt_clean = stmt
